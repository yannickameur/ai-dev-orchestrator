...
| DeepSeek | Claude Code, redirigé (`ANTHROPIC_BASE_URL`/`ANTHROPIC_API_KEY`) | `NOT VALIDATED` — désactivé par défaut (`config/workers.yaml`, worker `dana`). Nécessite une vraie `DEEPSEEK_API_KEY` (facturé à la consommation, jamais requis pour les autres providers) |
| Kimi | Claude Code, redirigé (`ANTHROPIC_BASE_URL`/`ANTHROPIC_API_KEY`) | `NOT VALIDATED` — désactivé par défaut (`config/workers.yaml`, worker `kai`). Nécessite une vraie `KIMI_API_KEY` (abonnement Kimi Code, jamais requis pour les autres providers) |
...
voter dans `ROADMAP.md` (section « Propositions à voter ») — ce ne sont ni
