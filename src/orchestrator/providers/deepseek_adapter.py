...
protocol. This module is therefore a thin factory that builds a
...
CLIs authenticate outside this project against a subscription and never
...
# DeepSeek's own docs before relying on this; not assumed stable forever.
...
def build_deepseek_adapter(
    *, env: Mapping[str, str] | None = None, **claude_code_adapter_kwargs: object
    ...
    key being present on the machine running the suite.
    ``DEEPSEEK_BASE_URL``/``DEEPSEEK_MODEL`` are optional overrides of the
    documented defaults; ``DEEPSEEK_API_KEY`` has no default and is
    required.
    """
    ...
    if not api_key or not api_key.strip():
        raise DeepSeekConfigError(
            ...
            "provider: it is not required for Claude, Codex, Mistral, or "
            f"Kimi to keep working. Set {ENV_API_KEY} only if you actually "
