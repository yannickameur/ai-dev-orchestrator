...
Kimi's own documentation (https://www.kimi.com/code/docs/en/,
...
``https://api.kimi.ai/coding/``: not a distinct CLI or wire protocol.
This module is therefore a thin factory that builds a
:class:`~orchestrator.providers.claude_code_adapter.ClaudeCodeAdapter`
configured for that redirect, rather than a second, independent adapter.
See that module's own docstring ("Reuse beyond real Anthropic") for the
mechanism.

Kimi Code is a subscription (quota-based, "Kimi Code" membership tiers),
not plain pay-as-you-go: the reason it was prioritized over a generic PAYG
integration. It still requires its own ``KIMI_API_KEY``, unlike Claude
Code/Codex/Vibe, whose CLIs authenticate outside this project and never
...
# Kimi's own docs before relying on this; not assumed stable forever.
# https://www.kimi.com/code/docs/en/third-party-tools/claude-code.html
...
def build_kimi_adapter(
    *, env: Mapping[str, str] | None = None, **claude_code_adapter_kwargs: object
    ...
    test passes an explicit mapping instead, so no test depends on a real
    key being present on the machine running the suite. ``KIMI_BASE_URL``/
    ...
    if not api_key or not api_key.strip():
        raise KimiConfigError(
            f"{ENV_API_KEY} is not set. Kimi is an optional, "
            "subscription-based provider: it is not required for Claude, "
            f"Codex, Mistral, or DeepSeek to keep working. Set {ENV_API_KEY} "
            "only if you actually want to use Kimi."
        )
